...
def clear_terminal():
    ...
    """
    os.system('cls' if os.name == 'nt' else 'clear')  # Windows ('cls') or others ('clear')
...
def play_game():
    ...
    while not all_ships_sunk(hidden_board):
        clear_terminal()  # Clear the screen for a fresh display
        print_visible_board(visible_board)  # Show the current state of the visible board
    ...
    clear_terminal()  # Final clear before announcing the victory
